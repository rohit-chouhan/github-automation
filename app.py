...
def follow(t):
    ...
    for i in range(limit):
        ...
        res = response.json()
        requests.put('https://api.github.com/user/following/'+res[utha]['login'], headers= {'Authorization' : 'Bearer '+t+''})
        print(" ("+str(i+1)+") Followed User Successfully: @"+res[utha]['login'])
        if utha==29:
            page+=1
            utha=0
        elif i==limit:
...
def unfollow(u,t):
    ...
    for i in range(limit):
        ...
        res = response.json()
        requests.delete('https://api.github.com/user/following/'+res[utha]['login'], headers= {'Authorization' : 'Bearer '+t+''})
        print(" ("+str(i+1)+") Unfollowed User Successfully: @"+res[utha]['login'])
        if utha==29:
            page+=1
            utha=0
        elif i==limit:
